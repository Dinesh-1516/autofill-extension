...
FOR DESCRIPTIVE FIELDS (LIKE 'About Me', 'Summary', 'Experience', etc.)
- For descriptive fields (like 'About Me', 'Summary', 'Experience', etc.), generate 20 lines of relevant content BASED ON the user's personal_details (e.g., use their occupation, education, skills, experience from personal_details to create coherent, personalized descriptions)
- If relvant information is not available in details, leave it EMPTY.
...
8. If a field is meant to be NULL or empty (e.g., optional fields with no matching data), DO NOT create any action for it
9. If a field's value would be NULL or empty string, skip it entirely - do not include it in the actions list
10. For optional fields where no matching data is found, leave them empty by not including them in the actions list
...
- Descriptive fields ('About Me', 'Professional Summary', 'Experience', 'Skills Summary'): Generate 3-4 lines using personal_details data (e.g., combine occupation + education + skills + years of experience into coherent paragraphs; use employment history for experience sections; highlight key skills and achievements)

IMPORTANT: 
- Focus ONLY on fields that need filling (should_fill: true). 
- Do not duplicate work already done by fuzzy matching.
- If a field is optional and no matching data is found, leave it empty by not including it in the actions list.
- Never include actions with NULL or empty string values - simply omit these fields from the actions list entirely.
...
Generate autofill actions ONLY for empty fields (should_fill: true) where you have valid, non-empty data to fill. 

IMPORTANT:
- Skip fields already filled by fuzzy matching
- Skip any field where the value would be NULL or empty string
- Only include fields in the actions list if you have actual content to fill
- If a field is optional and no matching data is found, simply omit it from the actions list
- Never include actions with NULL or empty values"""
